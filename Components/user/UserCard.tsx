import {Card, CardDescription, CardHeader, CardTitle} from '../../src/components/ui/card'
import {Button} from '../../src/components/ui/button'

type UserCardProps ={
    avatarUrl: string,
    name:string,
    bio:string,
    url:string
}

function UserCard({avatarUrl,name,bio,url}:UserCardProps) {
  return (
    <Card className='w-full lg:w-1/2 mb-8'>
        <CardHeader className='flex-row gap-x-8 items-center'>
            <img src={avatarUrl} alt={name} className='w-36 h-46 rounded-2xl object-cover' />
            <div className="flex flex-col gap-y-2">
                <CardTitle>{name || "Profile name isn't provided"}</CardTitle>
                <CardDescription>{bio || "Description isn't provided"}</CardDescription>
                <Button asChild size='sm' className='w-1/2 mt-2'>
                <a href={url} target="_blank" rel='noreferrer'>Follow</a>
                </Button>
            </div>
        </CardHeader>
    </Card>
  )
}

export default UserCard